...
printf("tid is%d, blockid is %d, blockdim is %d_thread id is%d griddim is %d\r\n", tid, blockIdx.x, blockDim.x, threadIdx.x,gridDim.x);
 float temp = 0; int cacheindex = threadIdx.x;
 while (tid < arraySize) { 
    temp = a[tid] * b[tid];   
...
int main()
{ 
   int a[arraySize] = { 1, 2, 3, 4, 5 };    int b[arraySize] = { 10, 20, 30, 40, 50 };
...
    // Launch a kernel on the GPU with one thread for each element.    
    addKernel<<<5, 4>>>(dev_c, dev_a, dev_b);
